...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if self.ant!=None and self.ant.container==True and self.ant.can_contain(insect):
                self.ant.contain_ant(insect)
            elif self.ant!=None and self.ant.container==False and insect.container==True:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            "*** YOUR CODE HERE ***"
            if self.ant.nonremovable==True and self.ant.constructor_count<=1:
                self.ant = self.ant
                insect.place = self
            else:
                self.ant = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while curr_place.entrance!=None:
            ...
            if len(curr_place.bees)==0:
                ...
                if curr_place!=hive:
                    if place_range>=self.min_range and place_range<=self.max_range:
...
class AntColony(object):
    ...
    """
    def __init__(self, strategy, hive, ant_types, create_places, food=10):
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if insect.watersafe==False and self.add_insect:
            insect.armor=insect.reduce_armor(insect.armor)
        return insect.armor
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor>0:
            if len(self.place.bees)!=0:
                list_of_bees=[b for b in self.place.bees]
                i=0
                while i<=len(list_of_bees)-1:
                    list_of_bees[i].armor=list_of_bees[i].armor-self.damage
                    if list_of_bees[i].armor==0:
                        self.place.bees.remove(list_of_bees[i])
                    i=i+1
                self.place.remove_insect(self)
...
class NinjaAnt(Ant):
    ...
    def action(self,colony):
        if self.place.bees!=None:
            list_of_bees=[bee for bee in self.place.bees]
            i=0
            while i<=len(list_of_bees)-1:
                list_of_bees[i].armor=list_of_bees[i].armor-self.damage
                if list_of_bees[i].armor==0:
...
class HungryAnt(Ant):
    ...
    def __init__(self):
        Ant.__init__(self)
        "*** YOUR CODE HERE ***"
        self.digesting=self.default

    def eat_bee(self, bee):
        "*** YOUR CODE HERE ***"
        if self.digesting==0 and len(self.place.bees)!=0:
           random_bee= random_or_none(self.place.bees)
           random_bee.armor-=random_bee.armor
           self.place.remove_insect(random_bee)
           self.digesting=self.time_to_digest

    def action(self, colony):
        "*** YOUR CODE HERE ***"
        if self.digesting!=0:
            print('Hungry Ant Digesting')
            self.digesting-=1
        else:
            if self.place!=None and self.place!=colony.hive:
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        ...
        self.ant=ant
        if self.place!=None:
            self.place.ant.ant=self.ant
        else:
            self.place=self.ant.place
            self.place.ant=self
            self.place.ant.ant=self.ant           
        
    def can_contain(self,ant):
        if ant.container==False and self.ant==None and self.container==True:
            return True
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor==0 and self.ant != None:
            ...
            print("Container ant armor is:",self.ant.armor)
...
class QueenAnt(ThrowerAnt):
    ...
    def __init__(self):
        ...
        "*** YOUR CODE HERE ***"
        self.constructor_counter()
        if self.constructor_count>=2:
            self.armor=0
            self.place.remove_insect(self)
        
        
    def constructor_counter(self):
        QueenAnt.constructor_count+=1
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        QueenAnt.Q_place=self.place
        QueenPlace.bees_method(self)
        if len(QueenPlace.bees)!=0:
            colony.queen=QueenPlace
        ThrowerAnt.action(self,colony)
        curr_place=self.place
        place_range=len(colony.places)
        list_of_places=[]
        while curr_place.exit!=None:
            curr_place=curr_place.exit
            list_of_places+=[curr_place,]
            place_range+=1
        
        for p in list_of_places:
            if p.ant!=None and p.ant not in self.damage_raised_ants:
                p.ant.damage+=p.ant.damage
                self.damage_raised_ants+=[p.ant,]
                
class QueenPlace(QueenAnt,AntColony):
    bees=[]

    def bees_method(self):
        if len(QueenAnt.Q_place.entrance.bees)!=0:
            return self.bees.append(bee for bee in QueenAnt.Q_place.bees)
