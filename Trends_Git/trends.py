...
def extract_words(text):
    ...
    n=0;i=0
    while n<=rng:
        if list(text)[n] not in ascii_letters:
            extract_char.append(' ')
            n+=1
        else:
            extract_char.append(text[n])
            n+=1
    extract_wrd=[]
    while i<=len(extract_char)-1:
        if i == 0:
            extract_wrd=extract_char[i]
        else:
            extract_wrd=str(extract_wrd)+extract_char[i]
        i+=1
    return extract_wrd.split()
...
def analyze_tweet_sentiment(tweet):
    ...
    "*** YOUR CODE HERE ***"
    wrds_in_tweet= extract_words(tweet_words(tweet))
    value=0;sentiment_count=0
    for n in range (0,len(wrds_in_tweet)):
       
        if get_word_sentiment(wrds_in_tweet[n])!=None:
            sentiment_count+=1
            value=value+get_word_sentiment(wrds_in_tweet[n])
    if sentiment_count==0:
        return None
    else:
        average=value/sentiment_count
        return average
...
def find_centroid(polygon):
    ...
    n=1;x_comp=0;y_comp=0;area_comp=0
    while n <= len(polygon)-1:
        a_pol_x=polygon[n-1][0]+polygon[n][0]
        comm_term=mul(polygon[n-1][0],polygon[n][1])-mul(polygon[n][0],polygon[n-1][1])
        a_pol_y=polygon[n-1][1]+polygon[n][1]
        x_comp+=mul(a_pol_x,comm_term) 
        y_comp+=mul(a_pol_y,comm_term)
        area_comp+=comm_term
        n+=1
    A_pol=0.5*area_comp
    if A_pol==0:
        ...
        C_x=(1/(6*A_pol))*x_comp
        C_y=(1/(6*A_pol))*y_comp
        return C_x,C_y,abs(A_pol)
...
def find_center(polygons):
    ...
    """
    "*** YOUR CODE HERE ***"
    n=0;sum_x=0;sum_y=0;sum_A=0
    while n <= len(polygons)-1:
        C_ix=find_centroid(polygons[n])[0]
        A_i=find_centroid(polygons[n])[2]
        sum_x+=C_ix * A_i
        sum_A+=A_i
        C_iy=find_centroid(polygons[n])[1]
        sum_y+=C_iy * A_i
        n+=1
    C_x=sum_x/sum_A
    C_y=sum_y/sum_A
...
def most_talkative_state(term):
    ...
    us_centers = {n: find_center(s) for n, s in us_states.items()}
    dir_states={}
    us_states_list=list(us_centers.keys())
    dir_states=group_tweets_by_state(tweets)

    curr_state={}
    n=0
    while n <= len(us_states_list)-1: 
        if us_states_list[n] in dir_states.keys():
            curr_state[us_states_list[n]]=len(dir_states[us_states_list[n]])
            n+=1
        else:
            n+=1
    max_value=max(curr_state.values())

    i=0
    while i <= len(us_states_list)-1:
        if us_states_list[i] in curr_state.keys():
            if curr_state[us_states_list[i]]==max_value:
                most_talkative_state=us_states_list[i]
                break
            else:
                i+=1
        else:
...
def average_sentiments(tweets_by_state):
    ...
    "*** YOUR CODE HERE ***"
    twitting_states=list(tweets_by_state.keys())
    i=0;sum=0
    while i <= len(twitting_states)-1:
        for n in range (0,len(tweets_by_state[twitting_states[i]])):
            if n==0:
                if analyze_tweet_sentiment(tweets_by_state[twitting_states[i]][n])==None:
                    averaged_state_sentiments[twitting_states[i]]=0
                else:
                    averaged_state_sentiments[twitting_states[i]]=analyze_tweet_sentiment(tweets_by_state[twitting_states[i]][n])
            else:
                if analyze_tweet_sentiment(tweets_by_state[twitting_states[i]][n])==None:
                    averaged_state_sentiments[twitting_states[i]]+=0
                else:
        ...
        averaged_state_sentiments[twitting_states[i]]/=len(tweets_by_state[twitting_states[i]])
        if averaged_state_sentiments[twitting_states[i]]==0:
            del averaged_state_sentiments[twitting_states[i]]
            i+=1
        else:
            i+=1
...
def group_tweets_by_hour(tweets):
    ...
    tweets_by_date={};tweets_by_date_f={}
    print(len(tweets))
    while n<=len(tweets)-1:
        
        date_of_tweet=datetime.date(tweet_time(tweets[n]))
       
        if date_of_tweet in tweets_by_date.keys():
            n+=1
        else:
            for i in range (0,len(tweets)):
                if datetime.date(tweet_time(tweets[i])) == date_of_tweet:
                    if date_of_tweet not in tweets_by_date.keys():
                        tweets_by_date[date_of_tweet]=[tweets[i],]
                    else:
                        tweets_by_date[date_of_tweet]+=[tweets[i],]
    ...
    date_list=list(tweets_by_date.keys())
    while m<= len(date_list)-1:
        while hour_start <= 23:
            time_of_tweet=datetime.time(tweet_time(tweets_by_date[date_list[m]][j]))
            if time_of_tweet.hour==hour_start:
                if hour_start not in tweets_by_hour.keys():
                    tweets_by_hour[hour_start]=[tweets_by_date[date_list[m]][j],]
                else:
                    tweets_by_hour[hour_start]+=[tweets_by_date[date_list[m]][j],]
            if j == len(tweets_by_date[date_list[m]])-1:
                hour_start+=1
                j=0
            else:
                j+=1
